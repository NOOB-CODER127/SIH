"""GPU Master Training & Evaluation Harness on 750+ Unified Real Case Documents.

Trains PyTorch CUDA / spaCy GPU NLP models on 750+ full-length real documents across all categories:
1. Multi-Page Police FIR Narratives
2. Supreme Court & High Court Judgments
3. Undercover Surveillance Field Logs
4. Call Detail Record (CDR) Intercept Transcripts
5. Financial Statements & FIU Suspicious Transaction Reports (STRs)
"""
...
UNIFIED_BENCHMARK_PATH = BASE_DIR / "data" / "master_unified_dataset" / "master_unified_benchmarks.json"
REGISTRY_PATH = BASE_DIR / "data" / "sample" / "registry.json"
UNIFIED_MODEL_OUTPUT_DIR = BASE_DIR / "artifacts" / "nlp_gpu_unified_master_model"
...
def check_gpu_environment():
    ...
    logging.info("================ UNIFIED MASTER GPU ENVIRONMENT ================")
...
def evaluate_by_doc_type(extractor: EntityExtractor, test_docs: list[dict]) -> dict:
    """Evaluates extraction metrics per document file category across unified dataset."""
    ...
    for test_case in test_docs:
        ...
        file_path = None
        for base_dir in ["case_files", "realistic_case_files", "huggingface_kaggle_datasets"]:
            for subdir in ["firs", "court_cases", "surveillance", "cdr_logs", "financial_logs", "court_judgments", "legal_ner_texts"]:
                candidate = BASE_DIR / "data" / base_dir / subdir / f"{doc_id}.txt"
                if candidate.exists():
                    file_path = candidate
                    break
            if file_path:
        ...
        gold_ents = test_case.get("gold_entities", [])
...
def run_unified_master_gpu_training():
    t0 = time.time()
    cuda_available, device_name = check_gpu_environment()

    if not UNIFIED_BENCHMARK_PATH.exists():
        logging.info("Unified dataset missing! Merging datasets...")
        from scripts.merge_all_real_datasets import merge_all_datasets
        merge_all_datasets()

    all_benchmarks = json.loads(UNIFIED_BENCHMARK_PATH.read_text())
    logging.info("Loaded Total Unified Real Dataset: %d case documents across all categories", len(all_benchmarks))
    ...
    train_unified_path = BASE_DIR / "data" / "master_unified_dataset" / "train_unified_benchmarks.json"
    train_unified_path.write_text(json.dumps(train_set, indent=2))

    # Phase 1: Heavy GPU Fine-Tuning
    logging.info("\n--- Phase 1: Heavy GPU Training on %d Multi-Source Case Documents ---", len(train_set))
    trainer = NLPTrainer(dataset_path=train_unified_path, output_dir=UNIFIED_MODEL_OUTPUT_DIR)
    trained_model_path = trainer.train_spacy_ner(iterations=30)
    ...
    report_path = BASE_DIR / "artifacts" / "unified_master_gpu_results.json"
    report_path.parent.mkdir(exist_ok=True)
    report_path.write_text(json.dumps(results, indent=2))
    logging.info("Saved Unified Master GPU Test Results to %s", report_path)


if __name__ == "__main__":
    run_unified_master_gpu_training()
